...
(function($) {
    $.fn.select2ToTree = function(options) {
        ...
        s2inst.on("select2:open", function(evt) {
            ...
            setTimeout(() => {
                open_options(evt.currentTarget);
            }, 20);
        });
    ...
    function open_options(target) {
        ...
        options.forEach(function(value) {
            do {
                var id = $('.select2-results__option[data-val="' + value + '"]').data('pup');
                value = id;
        ...
        unique.forEach(function(id) {
            var target = $('.select2-results__option[data-val="' + id + '"] .expand-collapse');
