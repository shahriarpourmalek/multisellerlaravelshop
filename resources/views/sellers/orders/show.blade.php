                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-5">
                                            <fieldset class="form-group">
                                                <select class="custom-select" id="shipping-status" data-action="{{ route('sellers.orders.shipping-status', ['order' => $order]) }}" {{ (!$order->hasPhysicalItem() || $order->status != 'paid' || $order->reserved() || $order->mainOrder) ? 'disabled' : '' }}>
                                            ...
                                            @endif
                                            @if ($order->mainOrder)
                                                <span>این سفارش به همراه سفارش شماره <a target="_blank" href="{{ route('orders.show', ['order' => $order->mainOrder]) }}">{{ $order->main_order_id }}</a> ارسال شده است.</span> <a href="{{ route('orders.show', ['order' => $order->mainOrder]) }}">مشاهده سفارش</a>
                                            @endif
                                        </div>
                                        <div class="col-md-7">
                                            <div class="float-right">
                                                @if (!$order->reserved())
                                                    <a href="{{ route('sellers.orders.shipping-form', ['order' => $order]) }}" target="_blank" class="btn btn-outline-info waves-effect waves-light"><i class="feather icon-printer"></i>چاپ برچسب پستی</a>
                                                @endif
                                                <a href="{{ route('sellers.orders.print', ['order' => $order]) }}" target="_blank" class="btn btn-outline-primary waves-effect waves-light"><i class="feather icon-printer"></i> چاپ</a>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            @foreach ($order->reservedOrders as $reserved_order)
                                                <div class="col-md-3">
                                                    <a href="{{ route('orders.show', ['order' => $reserved_order]) }}" target="_blank">سفارش شماره {{ $reserved_order->id }} <i class="feather icon-external-link"></i></a>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-sm" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="POST" action="{{ route('orders.destroy', ['order' => $order]) }}" id="product-delete-form">
                        @csrf
                        @method('delete')
